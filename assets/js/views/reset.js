...
export function renderReset(container, state) {
  ...
  resetCard.innerHTML = `
    <!-- Danger Icon Pod -->
    <div style="width: 76px; height: 76px; margin: 0 auto 20px; background: linear-gradient(135deg, rgba(254, 226, 226, 0.95) 0%, rgba(254, 202, 202, 0.6) 100%); border: 1.5px solid rgba(239, 68, 68, 0.3); border-radius: 22px; display: flex; align-items: center; justify-content: center; box-shadow: 0 14px 32px -6px rgba(239, 68, 68, 0.2), inset 0 1.5px 1.5px rgba(255, 255, 255, 1);">
      <svg class="pp-icon-anim-pulse" xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <h3 style="color: var(--pp-text-heading); font-size: 22px; font-weight: 800; margin: 0 0 10px 0; letter-spacing: -0.02em; font-family: var(--pp-font-heading);">Danger Zone: Factory Reset</h3>
    ...
    <div style="background: rgba(255, 255, 255, 0.85); border: 1px solid rgba(239, 68, 68, 0.18); border-radius: 16px; padding: 18px 20px; margin-bottom: 28px; text-align: left; box-shadow: inset 0 2px 4px rgba(0,0,0,0.01);">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 13px; color: var(--pp-text-main);">
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="color: var(--pp-danger); font-weight: 800;">✕</span> Meta, TikTok, GA4 & CAPI Tokens
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="color: var(--pp-danger); font-weight: 800;">✕</span> Custom Universal Tracker Rules
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="color: var(--pp-danger); font-weight: 800;">✕</span> Fraud Prevention Rules & Logs
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="color: var(--pp-danger); font-weight: 800;">✕</span> DataLayer & Event History
        </div>
      </div>
    </div>
    ...
    <!-- Step 1 Button -->
    <div id="step-1">
      <button id="btn-initiate-reset" class="pp-btn pp-btn-danger" style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%) !important; color: #ffffff !important; font-size: 14.5px; font-weight: 700; padding: 14px 28px; border-radius: 12px; width: 100%; border: none; box-shadow: 0 4px 16px rgba(239, 68, 68, 0.35); cursor: pointer; transition: all 0.25s ease;">
        Initiate Factory Reset
      </button>
    </div>

    <!-- Step 2 Confirmation Card -->
    <div id="step-2" style="display: none; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(16px); padding: 24px; border-radius: 18px; border: 1.5px solid rgba(239, 68, 68, 0.35); margin-top: 20px; text-align: left; box-shadow: 0 10px 30px -5px rgba(239, 68, 68, 0.15);">
      ...
      <button id="btn-final-reset" class="pp-btn pp-btn-danger" style="width: 100%; padding: 13px; font-size: 14px; font-weight: 700; opacity: 0.5; cursor: not-allowed;" disabled>
